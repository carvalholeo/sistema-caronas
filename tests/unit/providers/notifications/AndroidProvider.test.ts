...
// 2. Mockamos a biblioteca inteira, fornecendo nosso próprio objeto 'admin' falso.
jest.mock('firebase-admin', () => ({
  // A propriedade 'apps' agora é um array normal que podemos modificar nos testes.
  apps: [],
  // O método 'messaging' retorna um objeto que contém nossa função mockada.
  messaging: () => ({
...
describe('AndroidProvider', () => {
  ...
  describe('Constructor', () => {
    ...
    it('should not warn if Firebase Admin SDK is already initialized', () => {
      (mockedAdmin.apps as any[]).push({ name: 'mockApp' });
      new AndroidProvider();
