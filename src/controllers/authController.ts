...
class AuthController {
  ...
  public async generate2FA(req: Request, res: Response): Promise<Response> {
    try {
      ...
      return res
        .status(500)
        .json({ message: "Erro ao gerar segredo 2FA.", error: error.message });
    }
  }
  ...
  public async verify2FA(req: Request, res: Response): Promise<Response> {
    try {
      ...
      if (!decoded.twoFactorRequired)
        throw new Error("Token inválido para verificação 2FA.");

      const user = await UserModel.findById(decoded.id).select(
        "+twoFactorSecret",
      );
      if (!user || !user?.twoFactorSecret)
        throw new Error("Usuário não encontrado ou 2FA não configurado.");

      const isValid = authService.verifyTwoFactorCode(
