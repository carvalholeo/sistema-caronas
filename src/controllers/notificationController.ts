...
class NotificationController {
  ...
  public async subscribe(req: Request, res: Response): Promise<Response> {
    try {
      ...
      return res.status(500).json({
        message: "Erro ao registar a subscrição de notificação.",
        error: error.message,
      });
    }
  }
  ...
  public async updatePreferences(
    ...
    try {
      ...
      const updatedSubscription = await notificationService.updatePreferences(
        user,
        deviceIdentifier,
        preferencesData,
      );
      ...
      return res.status(500).json({
        message: "Erro ao atualizar as preferências de notificação.",
        error: error.message,
      });
    }
  }
