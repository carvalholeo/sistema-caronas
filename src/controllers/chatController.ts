...
class ChatController {
  public async getHistory(req: Request, res: Response): Promise<Response> {
    try {
      ...
      const history = await chatService.getChatHistory(rideId as unknown as Types.ObjectId, req.user!._id as unknown as Types.ObjectId);
  ...
  public async exportHistory(req: Request, res: Response): Promise<void> {
    try {
      const { rideId } = req.params;
      const txtContent = await chatService.exportChatHistoryAsTxt(rideId as unknown as Types.ObjectId, req.user!._id as unknown as Types.ObjectId);
