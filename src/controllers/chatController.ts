...
class ChatController {
  public async getHistory(req: Request, res: Response): Promise<Response> {
    try {
      ...
      const history = await chatService.getChatHistory(rideId as unknown as IRide, req.user!._id as unknown as IUser);
  ...
  public async exportHistory(req: Request, res: Response): Promise<void> {
    try {
      const { rideId } = req.params;
      const txtContent = await chatService.exportChatHistoryAsTxt(rideId as unknown as IRide, req.user!._id as unknown as IUser);
