...
class ChatController {
  ...
  public async exportHistory(req: Request, res: Response): Promise<void> {
    try {
      ...
      const txtContent = await chatService.exportChatHistoryAsTxt(
        rideId as unknown as IRide,
        user as unknown as IUser,
      );
      res.setHeader(
        "Content-disposition",
        `attachment; filename=chat_${rideId}.txt`,
      );
      ...
      res
        .status(500)
        .json({ message: "Erro ao exportar histórico.", error: error.message });
    }
  }
}
