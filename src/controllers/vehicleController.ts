...
class VehicleController {
  public async create(req: Request, res: Response): Promise<Response> {
    try {
      const vehicle = await vehicleService.createVehicle(req.user!._id as unknown as Types.ObjectId, req.body);
  ...
  public async getMyVehicles(req: Request, res: Response): Promise<Response> {
    try {
      const vehicles = await vehicleService.getVehiclesByOwner(req.user!._id as unknown as Types.ObjectId);
