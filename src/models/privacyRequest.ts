...
const PrivacyRequestSchema = new Schema<IPrivacyRequest>(
  {
    ...
    type: {
      type: String,
      required: true,
      enum: Object.values(PrivacyRequestType),
    },
    status: {
      type: String,
      enum: Object.values(PrivacyRequestStatus),
      default: PrivacyRequestStatus.REQUESTED,
    },
  ...
  if (this.isModified("status")) {
    if (this.isNew) {
      ...
      if (this.status !== PrivacyRequestStatus.REQUESTED) {
        return next(
          new Error(
            `Invalid initial status: ${this.status}. Must start as "requested"`,
          ),
        );
      }
      return next();
    ...
    const persisted = await (this.constructor as typeof PrivacyRequestModel)
      .findById(this._id)
      .select("status")
      .lean();
    ...
    switch (this.status) {
      ...
      case PrivacyRequestStatus.EXPIRED:
        ...
        if (this.requestedAt && this.completedAt < this.requestedAt) {
          return next(
            new Error("completedAt cannot be earlier than requestedAt"),
          );
        }
        break;
      default:
        ...
        if (
          this.status === PrivacyRequestStatus.REQUESTED ||
          this.status === PrivacyRequestStatus.INITIATED
        ) {
...
export const PrivacyRequestModel = model<IPrivacyRequest>(
  "PrivacyRequest",
