...
export const NotificationEventSchema = new Schema<INotificationEvent>(
  {
    scope: {
      ...
      default: NotificationScope.General,
      index: true,
    },
    ...
    statusHistory: [
      {
        ...
        details: {
          ...
          validate: {
            validator: validationFields,
            message:
              "Notification payload cannot contain sensitive information",
          },
        },
        _id: false,
      },
...
NotificationEventSchema.pre("validate", function (next) {
  ...
  if (!this.subscription && !this.user) {
    return next(
      new Error("Either subscription or user must be set for a notification"),
    );
  }
