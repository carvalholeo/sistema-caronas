...
BlockSchema.pre<IBlock>("validate", async function (next) {
  ...
  if (this.isNew) {
    if (this.status !== BlockStatus.APPLIED) {
      return next(
        new Error(
          `Invalid initial status: ${this.status}. Must start as "applied"`,
        ),
      );
    }
    if (
      ...
      this.invalidate(
        "blockedUser",
        "blockedUser must be different from blockerUser on insert",
      );
      return next(
        new Error("blockedUser must be different from blockerUser on insert"),
      );
    }
    return next();
  }
  ...
  const persisted = await (this.constructor as typeof BlockModel)
    .findById(this._id)
    .select("status")
    .lean();
  ...
  if (
    prevStatus === BlockStatus.REVERSED ||
    prevStatus === BlockStatus.REVERSED_BY_ADMIN
  ) {
    return next(
      new Error(`Block is terminal (${prevStatus}); status cannot change`),
    );
  }
