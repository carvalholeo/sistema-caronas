...
const RideSchema = new Schema<IRide>(
  {
    ...
    intermediateStops: [
      { location: { type: String }, point: { type: PointSchema } },
    ],
    departureTime: {
      type: Date,
      required: true,
...
RideSchema.index({
  "origin.point": "2dsphere",
  "destination.point": "2dsphere",
  departureTime: 1,
});
...
PassengerSchema.pre<IRidePassenger>("validate", async function (next) {
  ...
  if (this.isNew) {
    if (this.status !== PassengerStatus.Pending) {
      return next(
        new Error(
          'Invalid initial status. New passengers must start with status "pending"',
        ),
      );
    }
    ...
    if (originalRide) {
      const originalPassenger = originalRide.passengers.find((p) =>
        p.user.equals(this.user),
      );
      ...
      if (originalPassenger) {
        ...
        if (!allowed.includes(nextStatus)) {
          return next(
            new Error(
              `Invalid passenger status transition: ${prevStatus} -> ${nextStatus}`,
            ),
          );
        }
      }
...
RideSchema.pre<IRide>("validate", async function (next) {
  ...
  const approvedCount = this.passengers.filter(
    (p) => p.status === PassengerStatus.Approved,
  ).length;
  if (approvedCount > this.availableSeats) {
    return next(
      new Error(
        `Approved passengers (${approvedCount}) exceed available seats (${this.availableSeats})`,
      ),
    );
  }
  ...
  if (this.isModified("status")) {
    if (this.isNew) {
      if (this.status !== RideStatus.Scheduled) {
        return next(
          new Error(
            `Invalid initial status: ${this.status}. Must start as "scheduled"`,
          ),
        );
      }
    } else {
      const originalDoc = await RideModel.findById(this._id)
        .select("status")
        .lean();
  ...
  if (!this.isNew) {
    const hasBlockingPassengers = this.passengers.some((p) =>
      [PassengerStatus.Pending, PassengerStatus.Approved].includes(p.status),
    );
    const isWithinOneHour =
      this.departureTime.getTime() - Date.now() <= 60 * 60 * 1000;
    ...
    if (hasBlockingPassengers || isWithinOneHour) {
      ...
      const hasForbiddenChanges = modifiedPaths.some((path) =>
        forbiddenPaths.includes(path),
      );

      if (hasForbiddenChanges) {
        const reason = hasBlockingPassengers
          ? "while there are pending or approved passengers"
          : "within 1 hour before departureTime";
...
RideSchema.pre<IRide>("save", async function (next) {
  if (this.isModified("vehicle")) {
    const vehicle = await VehicleModel.findById(this.vehicle).select({
      status: 1,
      owner: 1,
    });
    ...
    if (vehicle.status !== VehicleStatus.Active) {
      return next(
        new Error("Vehicle must be active and approved to create rides"),
      );
    }

    if (
      vehicle.owner.toString() !==
      (this.driver._id as unknown as Schema.Types.ObjectId).toString()
    ) {
  ...
  if (this.isNew) {
    ...
    if (conflictingRide) {
      return next(
        new Error(
          "Driver already has a scheduled ride within one hour of this departure time.",
        ),
      );
    }
  }
  ...
  if (this.driver && this.departureTime) {
    ...
    if (count >= 4) {
      return next(
        new Error("A driver cannot have more than 4 rides on the same day"),
      );
    }
  }
